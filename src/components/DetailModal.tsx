...
export default function DetailModal() {
  ...
  const taskProviderName = taskProvider === 'fal' ? 'fal.ai' : taskProvider ? 'OpenAI' : t('common.unknown')
  const taskProfileName = task.apiProfileName || t('common.unknown')
  const taskModel = task.apiModel || t('common.unknown')
  ...
  const handleCopyError = async () => {
    ...
    try {
      ...
      showToast(getClipboardFailureMessage(t('task.copyErrorFailed'), err), 'error')
    }
  }
  ...
  const handleCopyPrompt = async () => {
    ...
    try {
      ...
      showToast(getClipboardFailureMessage(t('task.copyPromptFailed'), err), 'error')
    }
  }
  ...
  const handleCopyInputImage = async () => {
    ...
    try {
      ...
      console.error(err)
      showToast(getClipboardFailureMessage(t('task.copyReferenceFailed'), err), 'error')
    }
  }
    ...
    >
        ...
        <div ref={imagePanelRef} className="md:w-1/2 w-full h-64 md:h-auto bg-gray-100 dark:bg-black/20 relative flex items-center justify-center flex-shrink-0 min-h-[16rem]">
          ...
          {task.status === 'error' && isFalReconnecting && (
            <div className="w-full max-w-md px-4 text-center">
              ...
              <p className="text-sm font-medium text-yellow-500">{t('task.reconnecting')}</p>
            </div>
          )}
          {task.status === 'error' && !isFalReconnecting && (
            <div className="w-full max-w-md px-4 text-center">
                ...
                {task.rawImageUrls && task.rawImageUrls.length > 0 && (
                  <div className="relative group">
                    <button
                      ...
                      onClick={async (e) => {
                        if (task.rawImageUrls!.length === 1) {
                          ...
                          try {
                            ...
                            showToast(t('task.imageUrlCopied'), 'success')
                          } catch (err) {
                            showToast(getClipboardFailureMessage(t('task.copyImageUrlFailed'), err), 'error')
                          }
                        } else {
                          dismissAllTooltips()
                      ...
                      aria-label={t('task.copyImageUrl')}
                    >
          ...
          <div data-selectable-text className="flex-1">
            ...
            {showSourceInfo && (
              <div className="mb-2 rounded-lg bg-gray-50 px-3 py-2 text-xs dark:bg-white/[0.03]">
                <span className="text-gray-400 dark:text-gray-500">{t('task.source')}</span>
                <br />
        ...
        >
          <div ref={rawUrlsModalRef} className="flex w-full max-w-2xl max-h-[90vh] flex-col overflow-hidden rounded-2xl bg-white shadow-xl dark:bg-[#1c1c1e]" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4 dark:border-white/[0.08] shrink-0">
              <h3 className="text-base font-semibold text-gray-900 dark:text-white">{t('task.rawImageUrls', { count: rawImageUrls.length })}</h3>
      ...
      {showRawResponseModal && task?.rawResponsePayload && (
        ...
        >
          ...
          >
            <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4 dark:border-white/[0.08] shrink-0">
              <h3 className="text-base font-semibold text-gray-900 dark:text-white">{t('task.rawResponse')}</h3>
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={async () => {
