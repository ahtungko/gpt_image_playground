...
function parseResponsesImageResults(payload: ResponsesApiResponse, fallbackMime: string): Array<{
  ...
  if (!Array.isArray(output) || !output.length) {
    throw new Error('æŽ¥å£æœªè¿”å›žå›¾ç‰‡æ•°æ®')
  }
  ...
  if (!results.length) {
    throw new Error('æŽ¥å£æœªè¿”å›žå¯ç”¨å›¾ç‰‡æ•°æ®')
  }
...
async function callImagesApiConcurrent(opts: CallApiOptions, profile: ApiProfile, n: number): Promise<CallApiResult> {
  ...
  if (successfulResults.length === 0) {
    ...
    throw new Error('æ‰€æœ‰å¹¶å‘è¯·æ±‚å‡å¤±è´¥')
  }
...
async function callImagesApiSingle(opts: CallApiOptions, profile: ApiProfile): Promise<CallApiResult> {
  ...
  try {
    ...
    if (isEdit) {
      ...
      if (opts.maskDataUrl) {
        assertMaskEditFileSize('é®ç½©ä¸»å›¾æ–‡ä»¶', imageBlobs[0]?.size ?? 0)
        assertMaskEditFileSize('é®ç½©æ–‡ä»¶', maskBlob?.size ?? 0)
      }
      assertImageInputPayloadSize(
...
async function callResponsesImageApiSingle(opts: CallApiOptions, profile: ApiProfile): Promise<CallApiResult> {
  ...
  try {
    if (opts.maskDataUrl) {
      assertMaskEditFileSize('é®ç½©ä¸»å›¾æ–‡ä»¶', getDataUrlDecodedByteSize(inputImageDataUrls[0] ?? ''))
      assertMaskEditFileSize('é®ç½©æ–‡ä»¶', getDataUrlDecodedByteSize(opts.maskDataUrl))
    }
    assertImageInputPayloadSize(
