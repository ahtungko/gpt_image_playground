import type { ApiMode, ApiProfile, ApiProvider, AppSettings, KeyRole } from '../types'
...
function getDefaultBackgroundTasks(baseUrl: string) {
...
function normalizeKeyRole(value: unknown): KeyRole | null {
...
export function normalizeSettings(input: Partial<AppSettings> | unknown): AppSettings {
  ...
  const backgroundTasks = typeof record.backgroundTasks === 'boolean'
    ? record.backgroundTasks
    : getDefaultBackgroundTasks(active.baseUrl)
  ...
  return {
    ...
    language: record.language === 'en' || record.language === 'zh' ? record.language : getDefaultLocale(),
    profiles,
    activeProfileId,
    keyRole: normalizeKeyRole(record.keyRole),
    keyName: typeof record.keyName === 'string' ? record.keyName : '',
    keyGenerateRemaining: normalizeNullableNumber(record.keyGenerateRemaining),
    keyEditRemaining: normalizeNullableNumber(record.keyEditRemaining),
    keyMaxRunningTasks: normalizeNullableNumber(record.keyMaxRunningTasks),
  }
}
...
export const DEFAULT_SETTINGS: AppSettings = normalizeSettings({
  ...
  codexCli: false,
  apiProxy: false,
  backgroundTasks: getDefaultBackgroundTasks(DEFAULT_BASE_URL),
