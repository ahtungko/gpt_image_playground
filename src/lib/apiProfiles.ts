...
import { normalizeLocale, getDefaultLocale } from './i18n'
...
export function normalizeSettings(input: Partial<AppSettings> | unknown): AppSettings {
  ...
  return {
    ...
    backgroundTasks: typeof record.backgroundTasks === 'boolean' ? record.backgroundTasks : DEFAULT_BACKGROUND_TASKS,
    ...
    language: normalizeLocale(record.language ?? getDefaultLocale()),
    profiles,
    activeProfileId,
    keyRole: record.keyRole === 'admin' || record.keyRole === 'user' ? record.keyRole : null,
    keyName: typeof record.keyName === 'string' ? record.keyName : '',
    keyGenerateRemaining: typeof record.keyGenerateRemaining === 'number' && Number.isFinite(record.keyGenerateRemaining) ? record.keyGenerateRemaining : null,
    keyEditRemaining: typeof record.keyEditRemaining === 'number' && Number.isFinite(record.keyEditRemaining) ? record.keyEditRemaining : null,
    keyMaxRunningTasks: typeof record.keyMaxRunningTasks === 'number' && Number.isFinite(record.keyMaxRunningTasks) ? record.keyMaxRunningTasks : null,
  }
}
