...
async function syncLatestKeyProfile(settings: AppSettings): Promise<AppSettings> {
  try {
    ...
    const nextSettings = normalizeSettings(profile
      ? {
          ...settings,
...
export async function submitTask(options: { allowFullMask?: boolean; useCurrentApiProfileWhenReusedMissing?: boolean } = {}) {
  ...
  const paramsForRequest = {
    ...params,
    n: clampCountForSettings(params.n, requestSettings, currentTasks, { isEdit }),
  }
  const normalizedParams = normalizeParamsForSettings(paramsForRequest, requestSettings, { hasInputImages: isEdit })
  ...
  const taskId = genId()
  const backgroundTaskIds = canUseBackgroundImageTasks(requestSettings, normalizedParams, {
...
async function executeTask(taskId: string) {
  ...
  try {
    ...
    const callOptions = {
      ...
      maskDataUrl,
      onFalRequestEnqueued: (request: { requestId: string; endpoint: string }) => {
      ...
      onCustomTaskEnqueued: (request: { taskId: string }) => {
    ...
    if (latestTask.apiProvider === 'fal' && latestFalRequestInfo && isFalConnectionRecoverableError(err)) {
      ...
      const normalizedError = normalizeCaughtError(err, useStore.getState().settings.language)
      updateTaskInStore(taskId, {
        status: 'error',
        error: normalizedError.message || errorMessage,
...
export async function exportData(options: ExportOptions = { exportConfig: true, exportTasks: true }) {
  try {
    ...
    if (options.exportConfig) manifest.settings = exportableSettings(settings)
    if (options.exportTasks) {
